...
static NSString *const JK_ROUTER_WILDCARD_CHARACTER = @"~";

NSString *const JKRouterParameterURL = @"JKRouterParameterURL";
NSString *const JKRouterParameterCompletion = @"JKRouterParameterCompletion";
NSString *const JKRouterParameterUserInfo = @"JKRouterParameterUserInfo";
...
+ (void)registerURLPattern:(NSString *)URLPattern toHandler:(JKRouterHandler)handler {
...
+ (void)openURL:(NSString *)URL withUserInfo:(NSDictionary *)userInfo completion:(void (^)(id result))completion {
    ...
    if (parameters) {
        JKRouterHandler handler = parameters[@"block"];
        if (completion) {
            parameters[JKRouterParameterCompletion] = completion;
        }
        if (userInfo) {
            parameters[JKRouterParameterUserInfo] = userInfo;
        }
        if (handler) {
...
+ (id)objectForURL:(NSString *)URL withUserInfo:(NSDictionary *)userInfo {
    JKRouter *router = [JKRouter sharedIsntance];

    URL = [URL stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    NSMutableDictionary *parameters = [router extractParametersFromURL:URL];
    JKRouterObjectHandler handler = parameters[@"block"];
...
+ (void)registerURLPattern:(NSString *)URLPattern toObjectHandler:(JKRouterObjectHandler)handler {
    [[self sharedIsntance] addURLPattern:URLPattern andObjectHandler:handler];
}

- (void)addURLPattern:(NSString *)URLPattern andHandler:(JKRouterHandler)handler {
...
- (void)addURLPattern:(NSString *)URLPattern andObjectHandler:(JKRouterObjectHandler)handler {
    NSMutableDictionary *subRoutes = [self addURLPattern:URLPattern];
    if (handler && subRoutes) {
        subRoutes[@"_"] = [handler copy];
    }
...
- (NSMutableDictionary *)extractParametersFromURL:(NSString *)url {
    ...
    parameters[JKRouterParameterURL] = url;
    ...
    for (NSString *pathComponent in pathComponents) {
        ...
        for (NSString *key in subRoutesKeys) {
            if ([key isEqualToString:pathComponent] || [key isEqualToString:JK_ROUTER_WILDCARD_CHARACTER]) {
                found = YES;
...
- (NSArray *)pathComponentsFromURL:(NSString *)URL {
    ...
    if ([URL rangeOfString:@"://"].location != NSNotFound) {
        ...
        if ((pathSegments.count == 2 && ((NSString *) pathSegments[1]).length) || pathSegments.count < 2) {
            [pathComponents addObject:JK_ROUTER_WILDCARD_CHARACTER];
        }
